...
class _SVStoryComponentState extends State<SVStoryComponent> {
  ...
  getSocieties() async {
    await controller!.getSocietiesDetail();
  }

  FriendsStoriesController? controller;
  @override
  Widget build(BuildContext context) {
    controller ??= context.read<FriendsStoriesController>();
    return FutureBuilder(
        future: getSocieties(),
        builder: (context, snapshot) => controller!.isStoriesLoading
            ? SizedBox(
                                ...
                                return Column(
                                  children: [
                                    Container(
                                      ...
                                      child: controller!.societies[index]
                                                  .profileImage !=
                                              ""
                                          ? Image.network(
                                              IPHandle.profileimageAddress +
                                                  controller!.societies[index]
                                                      .profileImage
                                                      .validate(),
                                              height: 90,
                                              width: 80,
                                    ...
                                    10.height,
                                    Text(
                                        controller!.societies[index].name
                                            .validate(),
                                        style: secondaryTextStyle(
                                            size: 12,
                                            color: context.iconColor,
